...
### ルール設定
- **条件付き転送** — アプリごとに詳細な転送ルールを設定可能
- **フィルタリング** — タイトル・本文のキーワードマッチングで通知を制御

### 通知カスタマイズ
- **テンプレートエンジン** — 通知メッセージのフォーマットをカスタマイズ
- **Discord埋め込み対応** — リッチな埋め込み形式でのメッセージ送信
- **添付ファイル転送** — 通知に含まれる画像などを自動抽出・転送
...
### バックアップ・設定
- **設定バックアップ** — アプリ設定・ルールのバックアップ・リストア機能
- **自動バックアップ** — 定期的な自動バックアップをスケジュール実行
